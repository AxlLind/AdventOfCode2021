...
fn solve(universe: &Vec<String>, mut galaxies: Vec<(usize, usize)>, size: usize) -> usize {
  for r in (0..universe.len()).rev() {
    if universe[r].bytes().any(|c| c != b'.') {
      continue;
    }
  ...
  for c in (0..universe[0].len()).rev() {
    if (0..universe.len()).any(|r| universe[r].as_bytes()[c] != b'.') {
      continue;
    }
    for g in &mut galaxies {
      if g.1 > c {
...
fn main(input: &str) -> (usize, usize) {
  ...
  let galaxies = (0..universe.len())
    ...
    .filter(|(r,c)| universe[*r].as_bytes()[*c] == b'#')
    .collect::<Vec<_>>();

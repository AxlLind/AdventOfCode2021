...
fn simulate_polymer(init: &str, recipies: &HashMap<(char,char),char>, steps: usize) -> usize {
...
fn main(input: &str) -> (usize,usize) {
  ...
  let recipies = ingredients.lines()
    ...
    .collect::<HashMap<_,_>>();
  let p1 = simulate_polymer(init, &recipies, 10);
  let p2 = simulate_polymer(init, &recipies, 40);
  (p1,p2)
}

...
fn pass(pass: &[(char,i32)], board: &mut HashMap::<(i32,i32),char>, steps_map: &mut HashMap::<(i32,i32),u32>, c: char) {
  let (mut x, mut y) = (0,0);
  let mut steps = 0;
  for (way, amount) in pass {
...
fn main() {
  ...
  board.insert((0,0), 'O');
  pass(&PASS_ONE[..], &mut board, &mut steps_map1, '#');
  pass(&PASS_TWO[..], &mut board, &mut steps_map2, '%');
  let len = board.iter()
    .filter(|(_,c)| **c == 'X')
    .fold(100000, |len, ((x,y),_)| {
      let pos = (*x,*y);
      let d1 = steps_map1.get(&pos).unwrap();
      let d2 = steps_map2.get(&pos).unwrap();
      cmp::min(len, d1 + d2)
    });

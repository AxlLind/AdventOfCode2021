...
fn find_goal_path(map: &HashMap<(i64, i64), (i64)>, x: i64, y: i64, ex: i64, ey: i64) -> Vec<(i64,i64)> {
  ...
  let mut target = loop {
    ...
    for (dx,dy) in &neighbors {
      let (vx,vy) = (ux + dx, uy + dy);
      let v = (vx,vy);
      if q.contains(&v) && *map.get(&v).unwrap() != 0 {
        let alt = dist.get(&u).unwrap() + 1;
        if alt < *dist.get(&v).unwrap() {
          dist.insert((vx, vy), alt);
          prev.insert((vx, vy), Some(u));
        }
      }
    }
  };
  ...
  let mut s = Vec::new();
  loop {
    let (ux,uy) = target.unwrap();
    target = *prev.get(&(ux,uy)).unwrap();
    if target.is_none() {
      break;
    }
    let (vx,vy) = target.unwrap();
    s.push((ux - vx, uy - vy));
  }
  s
}
...
fn main() {
  ...
  print_map(&map);
  let answer = find_goal_path(&map, 0, 0, gx, gy).len() + 1;
